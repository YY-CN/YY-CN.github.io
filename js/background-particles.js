(function() {
  ...
  // 粒子颜色跟随主题色板（--tc-particle-rgb，由 theme-colors.js 切换）。
  function refreshPalette() {
    var rgb = getComputedStyle(document.documentElement)
      .getPropertyValue('--tc-particle-rgb')
      .replace(/\s+/g, ' ')
      .trim();
    if (!rgb) return;
    palette.line = rgb;
    palette.point = 'rgba(' + rgb + ', .42)';
  }

  document.addEventListener('theme-color-change', refreshPalette);
  refreshPalette();
